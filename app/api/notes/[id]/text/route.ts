/**
 * POST /api/notes/:id/text - Get presigned URL for text upload to note
 */
...
import { parseBody, addTextBodySchema } from '@/lib/validation';
...
export async function POST(
  ...
  try {
    ...
    await checkRateLimit(`text:${userId}`, rateLimits.presign);
    ...
    const body = await parseBody(request, addTextBodySchema);
    ...
    if (error || !data) {
      console.error('[notes/text] Failed to create signed URL:', error);
